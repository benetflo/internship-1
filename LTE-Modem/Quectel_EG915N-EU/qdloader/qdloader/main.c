/******************************************************************************
  ...
  ---------------------------------------------------------------------------
  Copyright (c) 2016 - 2021 Quectel Wireless Solution, Co., Ltd.  All Rights
Reserved. Quectel Wireless Solution Proprietary and Confidential.
  ---------------------------------------------------------------------------
******************************************************************************/
#include "std_c.h"
...
uint8_t g_nv_src_buffer[BURN_MAX_SIZE];
uint8_t g_nv_dst_buffer[BURN_MAX_SIZE];
char g_backup_file_path[256];
char g_nv_fix_path[256];
FILE *out_fp = NULL;
...
static struct quectel_modem_info quectel_modems[] = {
    ...
    {"RG225A", "2c7c/600b/", 3, "AT+QDownLOAD=1\r"},
    {NULL, NULL, 0, NULL},
};
...
const struct quectel_modem_info *find_quectel_modem(const char *PRODUCT)
{
    ...
    while (quectel_modems[i].modem_name)
    {
        ...
        {
            if (strstr(PRODUCT, "202") && (strstr(PRODUCT, "1782") && strstr(PRODUCT, "4d00"))) // UIS8310 -- 1782/4d00/202   //UDX710 --
                                                                                                // 1782/4d00/2430
            {
                if (strncasecmp(quectel_modems[i].modem_name, "EC200D",
                                6)) // EC200D is 8310, RG500 is udx710, so need to distinguish
                {
                    i++;
                    continue;
                }
            ...
            if (strstr(PRODUCT,
                       "2c7c")) // The State Grid module cannot forcibly specify the
                                // diag interface value in emergency download mode
            {
                if ((!strncasecmp(modem_name_para, "EC200T", 6) || !strncasecmp(modem_name_para, "EC200A", 6)) ||
                    ...
                    (!strncasecmp(modem_name_para, "EC200N", 6) || !strncasecmp(modem_name_para, "EC200S", 6)) || (!strncasecmp(modem_name_para, "EC200D", 6)))
                {
                    if (usb_interface >= 0) quectel_modems[i].usb_interface = usb_interface;
                }
            }
...
{
    printf("%s -f fw_image\n", self);
    printf("for EC200U, fw image is '.pac',         can be found in the FW "
           "package\n");
    printf("for RG500U, fw image is '.pac',         can be found in the FW "
           "package\n");
    printf("for EC200S, fw image is 'firmware.bin', can be found in the FW "
           "package\n");
    printf("for EC200T, fw image is 'BinFile.bin',  create by windows tool "
           "'FBFMake.exe -r update.blf -f output', .blf file can be found in the "
           "FW package \n");
    printf("for RM500K/RG500L, fw image is 'NULL', sysupgrade.bin must be placed "
           "in the adb /tmp/ directory, the -b argument is required, like "
           "'./QDloader -b' \n");
    printf("for the state grid module, the module model needs to be specified, "
           "such as '-g EC200T' \n");
    printf("for backup NV, default store in the /tmp directory, '-x' is required "
           "to specify the path, such as '-x /root/users' \n");
...
{
    if (g_specify_port[0] == '\0' || strncasecmp(g_specify_port, "/dev", 4) != 0) return -1;
    FILE *fp = NULL;
    char cmd_buf[80] = {0};
    char recv_buf[256] = {0};
    ...
    /*
        /usb1/1-9/1-9.4/1-9.4.4/1-9.4.4.4/1-9.4.4.4:1.3/ttyUSB6/tty/ttyUSB6
        /usb1/1-9/1-9.4/1-9.4.4/1-9.4.4:1.3/ttyUSB6/tty/ttyUSB6
        /usb1/1-9/1-9.4/1-9.4:1.3/ttyUSB6/tty/ttyUSB6
        /usb1/1-9/1-9:1.3/ttyUSB6/tty/ttyUSB6
    */
    if (ptr[9] == '/')
    {
        if (ptr[13] == ':')
        {
            if (ptr_len < 14) return -1;
    ...
    /*
        /usb1/1-12/1-12.4/1-12.4.4/1-12.4.4.4/1-12.4.4.4:1.3/ttyUSB6/tty/ttyUSB6
        /usb1/1-12/1-12.4/1-12.4.4/1-12.4.4:1.3/ttyUSB6/tty/ttyUSB6
        /usb1/1-12/1-12.4/1-12.4:1.3/ttyUSB6/tty/ttyUSB6
        /usb1/1-12/1-12:1.3/ttyUSB6/tty/ttyUSB6
    */
    else if (ptr[10] == '/')
    {
        if (ptr[15] == ':')
        {
            if (ptr_len < 15) return -1;

            strncpy(g_specify_port, ptr + 11, 4);
            dprintf("g_specify_port:%s\n", g_specify_port);
        }
        else if (ptr[15] == '.')
        {
            ...
            if (ptr[24] == ':')
            {
                strncpy(g_specify_port, ptr + 18, 6);
                dprintf("g_specify_port:%s\n", g_specify_port);
            }
            else if (ptr[24] == '.')
            {
                if (ptr[35] == ':')
                {
                    strncpy(g_specify_port, ptr + 27, 8);
                    dprintf("g_specify_port:%s\n", g_specify_port);
                }
                else if (ptr[35] == '.')
                {
                    if (ptr[48] == ':')
                    {
                        strncpy(g_specify_port, ptr + 38, 10);
                        dprintf("g_specify_port:%s\n", g_specify_port);
                    }
                    else if (ptr[48] == '.')
                    {
...
#endif
    dprintf("Version: QDloader_Linux_Android_V1.0.14\n");

    optind = 1; // call by popen(), optind mayby is not 1
    while (-1 != (opt = getopt(argc, argv, "s:f:u:g:x:ebvhl")))
    {
        switch (opt)
        {
            case 's':
                ...
                dprintf("g_specify_port:%s\n", g_specify_port);
                break;
            case 'f': fw_file = strdup(optarg); break;
            case 'u':
                usbmon_logfile = strdup(optarg);
                dprintf("usbmon_logfile:%s\n", usbmon_logfile);
                break;
            case 'x': backup_file_path = strdup(optarg); break;
            case 'g':
            ...
            case 'l':
                low_memory_mode++;
                dprintf("tool will in low momory mode l=%d\n", low_memory_mode);
                break;
            default: return usage(argv[0]); break;
        }
    }
...
_repair_modem:
    ...
    if (backup_file_path)
    {
        if (access(backup_file_path, F_OK) && errno == ENOENT) mkdir(backup_file_path, 0755);
...
_scan_modem:
    ...
    if (ret == 10) goto _repair_modem;

    if (ret == 0) update_transfer_bytes(0xffffffff);
