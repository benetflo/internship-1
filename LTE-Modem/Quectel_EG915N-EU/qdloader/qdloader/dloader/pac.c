...
int pac_read_bin(pac_ctx_t *ctx, void *pBuf, size_t nSize)
{
    ...
    if ((ctx->cur_bin_offset + nSize) > bin_hdr->dwLoFileSize) nSize = bin_hdr->dwLoFileSize - ctx->cur_bin_offset;

    if (nSize == 0) return 0;

    ret = fread(pBuf, 1, nSize, fp);
    if (ret > 0) ctx->cur_bin_offset += ret;
    return ret;
}
...
int xml_parse_NVBackup(pac_ctx_t *ctx, const char *xml_s, const char *xml_e)
{
    ...
    size_t n, nNVItem;
    uint32_t NVid;
    char NVItem_backup[2] = "0";

    pNVItem = xml_s;
    while (pNVItem < xml_e)
    {
        p = xml_find_key_value(pNVItem, xml_e, "backup", &n);
        if (!p) goto out;
        strncpy(NVItem_backup, p, min(n, sizeof(NVItem_backup)));
...
int xml_parse_Partition(pac_ctx_t *ctx, const char *xml_s, const char *xml_e)
{
    ...
    if (Partition.PartitionSize != 0xFFFFFFFF) Partition.PartitionSize = Partition.PartitionSize * 1024 * 1024;
...
int xml_parse_File(pac_ctx_t *ctx, const char *xml_s, const char *xml_e, char backup)
{
    ...
    if (p) strncpy(File.Base, p, min(n, sizeof(File.Base)));
    ...
    if (!strcasecmp(File.Type, "EraseFlash") || !strcasecmp(File.Type, "EraseFlash2"))
    {
        if (File.CheckFlag[0] == '0')
        {
            p = xml_find_node_value(xml_s, xml_e, "CheckFlag", &n);
    ...
    {
        if (File.CheckFlag[0] == '0') // pac File.CheckFlag[0] only = 0 or = 1
        {
            p = xml_find_node_value(xml_s, xml_e, "CheckFlag", &n);
            goto_out(!p);
            strncpy(File.CheckFlag, p, min(n, sizeof(File.CheckFlag)));
...
static int pac_parser_xml(pac_ctx_t *ctx)
{
    ...
    if (!bin_hdr) bin_hdr = pac_lookup_FileID(ctx, "");

    if (!bin_hdr) goto out;

    if (pac_open_bin(ctx, bin_hdr)) goto out;

    ctx->xml_len = bin_hdr->dwLoFileSize;
    ctx->xml_buf = (char *)malloc(ctx->xml_len + 32);
    if (!ctx->xml_buf) goto out;

    ctx->xml_len = pac_read_bin(ctx, ctx->xml_buf, ctx->xml_len);
    if (ctx->xml_len < 1024) goto out;
...
int pac_parser(pac_ctx_t *ctx, FILE *fw_fp)
{
    ...
    if (n != sizeof(struct pac_header_t)) return -1;

    if (le32toh(pachdr->nFileCount) > 64) return -1;

    n = fread(ctx->bin_hdr, 1, sizeof(struct bin_header_t) * le32toh(pachdr->nFileCount), fp);
    if (n != (sizeof(struct bin_header_t) * le32toh(pachdr->nFileCount))) return -1;
    ...
    for (n = 0; n < le32toh(pachdr->nFileCount); n++)
    {
        ...
        if (verbose > 0)
            dprintf("idx: %zd, FileID: %s, FileName: %s, szFileVersion: %s, Size: %u, "
                    "_nFileFlag:%u,_nCheckFlag:%u\n",
                    n, (char *)(bin_hdr->szFileID), (char *)(bin_hdr->szFileName), (char *)bin_hdr->szFileVersion, bin_hdr->dwLoFileSize, bin_hdr->_nFileFlag,
                    bin_hdr->_nCheckFlag);
#endif
    }
