...
struct bin_header_t
{
    ...
    uint32_t _nFileFlag;         // if "0", means that it need not a file, and
                                 // it is only an operation or a list of operations, such as file ID is
                                 // "FLASH" if "1", means that it need a file
...
struct pac_header_t
{
    ...
    uint32_t _dwFileOffset;     // the offset from the packet file header to the array of FILE_T struct
                                // buffer
    uint32_t _dwMode;
...
extern int nvMergeFileLowMemory(const char *src_file, pac_ctx_t *ctx, Scheme_t *File);
