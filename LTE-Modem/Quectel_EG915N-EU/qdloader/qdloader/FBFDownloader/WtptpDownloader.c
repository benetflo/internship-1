...
#define _T
static const NOTIFICATIONTABLE WTPTPNotificationMsg[] = {{PlatformBusy, _T("Platform is busy")},
...
const struct stFlatformBootLoaderInfo gstFlatformBLInfoSet[] = {{PLAT_ULC2, ULC2_BOOTLOADER_DATE, ULC2_BOOTLOADER_PROCESSOR, ULC2_BOOTLOADER_VERSION},
...
static int Data(struct CBinFileWtp *me, WTPSTATUS *pWtpStatus, uint8_t *pData, UINT32 Length, int isLastData, int fmode)
{
    ...
    if (isLastData) dprintf("WtpCmd: %s Length=%u, isLastData=%d\n", __func__, Length, isLastData);
...
static int WtptpDownLoad_DownloadImage(struct CBinFileWtp *me, WTPSTATUS *pWtpStatus, int DeviceType, struct fbf_file_params *fbf_final_file_pa)
{
    ...
    if (ImageType == TIMH || ImageType == DKBI)
    {
        switch (ImageType)
        {
            case TIMH:
                ...
                break;
            case DKBI: szFileName = g_DkbFilePath; break;
            default:
    ...
    if (kBootLoader == DeviceType || PLAT_EDEN_A0 == me->m_ePlatformType || PLAT_HELAN2 == me->m_ePlatformType || PLAT_ULC1 == me->m_ePlatformType ||
        PLAT_HELAN3 == me->m_ePlatformType || PLAT_HELAN4 == me->m_ePlatformType)
    {
        // bFastDownload = TRUE;
        fmode = 1;
    }
...
EDeviceType WtptpDownLoad_GetDeviceType(struct CBinFileWtp *me, ePlatForm ePlatfromType, const char *strDate, const char *strVersion, const char *strProcessor)
{
    ...
    for (idx = 0; idx < sizeof(gstFlatformBLInfoSet) / sizeof(gstFlatformBLInfoSet[0]); ++idx)
    {
        ...
        {
            if ((NULL != strstr(strDate, gstFlatformBLInfoSet[idx].strBLDate)) && (NULL != strstr(strProcessor, gstFlatformBLInfoSet[idx].strBLProcessor)) &&
...
int WtptpDownLoad_GetDeviceBootType(struct CBinFileWtp *me, struct fbf_file_params *fbf_file_pa, struct fbf_file_params *fbf_final_file_pa)
{
        ...
        for (k = 0; k < fbf_file_pa->FBF_timheader_bin_count; k++)
        {
            ...
            if (ptr != NULL)
            {
                if (!strncasecmp(ptr + 2, swd_ddr_vendor_id_str, 4))
                {
                    ...
                    memmove(fbf_final_file_pa->fbf_fl[fbf_final_file_pa->FBF_timheader_bin_count].FBF_timheader_bin, fbf_file_pa->fbf_fl[k].FBF_timheader_bin,
                            strlen(fbf_file_pa->fbf_fl[k].FBF_timheader_bin));
        ...
        for (k = 0; k < fbf_file_pa->FBF_h_bin_count; k++)
        {
            ...
            if (ptr != NULL)
            {
                if (!strncasecmp(ptr + 2, swd_ddr_vendor_id_str, 4))
                {
                    ...
                    memmove(fbf_final_file_pa->fbf_fl[fbf_final_file_pa->FBF_h_bin_count].fbf_fi_info.FBF_h_bin, fbf_file_pa->fbf_fl[k].fbf_fi_info.FBF_h_bin,
                            strlen(fbf_file_pa->fbf_fl[k].fbf_fi_info.FBF_h_bin));
    ...
    else if (device_type == 2)
    {
        ...
        while (1)
        {
            ...
            if ((pWtpStatus->Flags & 0x1) != MESSAGEPENDING) usleep(500 * 1000);
        }
