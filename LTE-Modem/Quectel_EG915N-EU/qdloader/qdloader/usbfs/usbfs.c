...
int recvSync(int usbfd, void *data, size_t len, unsigned timeout)
{
    ...
    ret = ioctl(usbfd, USBDEVFS_BULK, &bulk);
    if (ret == -1 && errno != ETIMEDOUT) printf("bluk in = %d, errno: %d (%s)\n", ret, errno, strerror(errno));

    if (ret > 0) dump_data(data, ret, '<');
    return ret;
}
...
int usbfs_open(const USBFS_T *usbfs_dev, int interface_no)
{
    ...
    if (usbfs_decode(usbfd, interface_no) == -1)
    {
        ...
        close(usbfd);
#ifdef EC200T_SHINCO
        if (strStartsWith(usbfs_dev->PRODUCT, "2c7c/6026/"))
        { // EC200T
            tcp_sockfd = connect_tcp_server("192.168.225.1", 10080);
            if (tcp_sockfd != -1) fcntl(tcp_sockfd, F_SETFL, fcntl(tcp_sockfd, F_GETFL) | O_NONBLOCK);
            return tcp_sockfd;
        }
#endif
        return -1;
    ...
    if (strStartsWith(usbfs_dev->PRODUCT, "1782/4d00/"))
    { // RG500U
        sciu2sMessage(usbfd);
    }
...
{
#ifdef EC200T_SHINCO
    if (tcp_sockfd == usbfd) tcp_sockfd = -1;
#endif
    close(usbfd);
    return 0;
}
...
static int quectel_get_sysinfo_by_uevent(const char *uevent, UEVENT_T *pSysInfo)
{
    ...
    while (fgets(line, sizeof(line), fp))
    {
        ...
        {
            if (pSysInfo) strncpy(pSysInfo->DEVNAME, &line[strlen("DEVNAME=")], sizeof(pSysInfo->DEVNAME));
        }
...
{
    if (module_dev_path[0] == '\0' || strncasecmp(module_dev_path, "/dev", 4) != 0) return -1;
    FILE *fp = NULL;
    char cmd_buf[320] = {0};
    char recv_buf[256] = {0};
    char buf_tmp[256] = {0};
    ...
    else
    {
        ...
        snprintf(buf_tmp, sizeof(buf_tmp), "%s, %s", itoa(MAJOR, itoa_major_buf, 10), itoa(MINOR, itoa_minor_buf, 10));
        if (strstr(recv_buf, buf_tmp)) return 1;
    }
...
int usbfs_find_quectel_modules(USBFS_T *usbfs_dev, size_t max, int usb3_speed, struct timespec usb3_process_start_time)
{
    ...
    while ((de = readdir(busdir)))
    {
        ...
        if (!sysinfo.MAJOR || !sysinfo.PRODUCT[0] || !sysinfo.DEVTYPE[0]) continue;

        if (sysinfo.MAJOR != 189 || strcmp(sysinfo.DEVTYPE, "usb_device")) continue;
        ...
        if (find_quectel_modem(sysinfo.PRODUCT))
        {
            ...
            snprintf(usbfs_dev[count].sys_path, sizeof(usbfs_dev[count].sys_path), "%s/%.16s", base, de->d_name);
            if ('\0' != sysinfo.DEVNAME[0]) snprintf(usbfs_dev[count].dev_name, sizeof(usbfs_dev[count].dev_name), "/dev/%.16s", sysinfo.DEVNAME);
            ...
            {
                if (strstr(sysinfo.PRODUCT, "1782") || strstr(sysinfo.PRODUCT, "525") || strstr(sysinfo.PRODUCT, "1286") || strstr(sysinfo.PRODUCT, "2ecc"))
                {
                    int speed;
    ...
    if (modem_require_count == 1)
    {
        ...
        if (usb3_process_start_time.tv_sec > this_atime[0].tv_sec ||
            (usb3_process_start_time.tv_sec == this_atime[0].tv_sec && usb3_process_start_time.tv_nsec > this_atime[0].tv_nsec))
        {
    ...
    else if (modem_require_count > 1)
    {
        ...
        for (i = 0; i < modem_require_count - 1; i++)
        {
            ...
            {
                if (this_atime[k].tv_sec > this_atime[k + 1].tv_sec || (this_atime[k].tv_sec == this_atime[k + 1].tv_sec && this_atime[k].tv_nsec > this_atime[k + 1].tv_nsec))
                {
        ...
        if (usb3_process_start_time.tv_sec > this_atime[modem_require_count - 1].tv_sec ||
            (usb3_process_start_time.tv_sec == this_atime[modem_require_count - 1].tv_sec && usb3_process_start_time.tv_nsec > this_atime[modem_require_count - 1].tv_nsec))
        {
            count = 0;
            memset(&usbfs_dev[count], 0, sizeof(USBFS_T));
        }
...
{
    if (usbmon_logfile_fd > 0) close(usbmon_logfile_fd);
    if (usbmon_fd > 0) close(usbmon_fd);
}
