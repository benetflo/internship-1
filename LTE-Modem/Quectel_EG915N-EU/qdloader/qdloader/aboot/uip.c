...
static uint16_t uip_icmp6chksum(struct uip_ip_hdr *ip_hdr) { return upper_layer_chksum(ip_hdr); }

static uint16_t uip_udpchksum(struct uip_ip_hdr *ip_hdr) { return upper_layer_chksum(ip_hdr); }
...
int ethos_write_aboot_cmd(aboot_t *aboot, const uint8_t *cmd, size_t len) { return ethos_write_aboot_data(aboot, cmd, len, 1); }
