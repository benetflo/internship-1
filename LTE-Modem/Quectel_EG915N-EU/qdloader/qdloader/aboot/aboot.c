...
{
    if (sendSync(aboot->usb.fd, buf, len, 0) == len) return 0;
    return -1;
}
...
static void preamble_callback(struct aboot *aboot, const uint8_t *data, size_t size)
{
    ...
    if (aboot->aboot_device_type != ABOOT_DEVICE_TYPE_NONE) return;

    for (i = 0; (i + 4) < size; i++)
    {
        if (data[i] != 0x7e) continue;

        if (data[i + 1] == 0x7e && !strncmp((char *)data + 2, ABOOT_PREAMBLE_UUUU, ABOOT_PREAMBLE_SIZE))
        {
...
static int preamble_start(aboot_t *aboot)
{
    ...
    for (t = 0; t < 2000; t += 100)
    {
        // if (t == 0 || t == 1000)
        {
            if (serial_port_write(aboot, buffer, ABOOT_PREAMBLE_SIZE + 1)) break;
        }

        msleep(100);
        if (aboot->aboot_device_type != ABOOT_DEVICE_TYPE_NONE) break;
    }
...
static void *serial_port_read(void *param)
{
    ...
    while (1)
    {
        ...
        if (aboot->process_incoming_data) aboot->process_incoming_data(aboot, aboot->usb.rxbuf, ret);
    }
...
static int aboot_transport_send_cmd(aboot_t *aboot, const uint8_t *cmd, size_t size)
{
    ...
    {
        case ABOOT_DEVICE_TYPE_SMUX: ret = cmd ? smux_write_aboot_cmd(aboot, cmd, size) : 0; break;

        case ABOOT_DEVICE_TYPE_ETHOS: ret = ethos_write_aboot_cmd(aboot, cmd, size); break;
    ...
    {
        ret = pthread_cond_timeout_np(&cmd_cond, &cmd_mutex, 10000); // In the case of weak USB capabilities, it is necessary to increase the waiting time for response; When there
                                                                     // are many responses, it is necessary to increase the waiting time, such as EC600MCNLA R06A07 STASR1606-1065
        pthread_mutex_unlock(&cmd_mutex);
        if ((ret == 0 && aboot->cmd_response[0] == 0) && aboot->aboot_device_type == ABOOT_DEVICE_TYPE_ETHOS)
        {
    ...
    if (!memcmp(aboot->cmd_response, "protocol error", 14)) return -1;
...
int aboot_download_process(aboot_t *aboot)
{
    ...
    {
        if (!strcmp(cmd_line, cmd_call) && is_crane_bootrom && aboot->aboot_device_type == ABOOT_DEVICE_TYPE_SMUX)
        {
