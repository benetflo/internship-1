...
static void ethos_end_of_frame(aboot_t *aboot)
{
    switch (aboot->frametype)
    {
        case ETHOS_FRAME_TYPE_DATA:
            ...
            {
                if (aboot->framesize == ETHOS_PREAMBLE_SIZE && !strncmp((const char *)aboot->rxbuf, ETHOS_PREAMBLE_UUUU, ETHOS_PREAMBLE_SIZE))
                {
        ...
        case ETHOS_FRAME_TYPE_TEXT:
            ...
            break;
        case ETHOS_FRAME_TYPE_HELLO: break;
        case ETHOS_FRAME_TYPE_HELLO_REPLY:
            ...
            break;
        case ETHOS_FRAME_TYPE_UPLOAD_PROG: break;
        case ETHOS_FRAME_TYPE_HEART_BEAT: break;
        default: break;
    }
    ...
    for (i = 0; i < len; i++)
    {
        ...
        switch (ethos->state)
        {
            ...
            case IN_ESCAPE:
                switch (c)
                {
                    case (ETHOS_FRAME_DELIMITER ^ 0x20): ethos_handle_char(ethos, ETHOS_FRAME_DELIMITER); break;
                    case (ETHOS_ESC_CHAR ^ 0x20): ethos_handle_char(ethos, ETHOS_ESC_CHAR); break;
                    case (ETHOS_FRAME_TYPE_TEXT ^ 0x20): ethos->frametype = ETHOS_FRAME_TYPE_TEXT; break;
                    case (ETHOS_FRAME_TYPE_HELLO ^ 0x20): ethos->frametype = ETHOS_FRAME_TYPE_HELLO; break;
                    case (ETHOS_FRAME_TYPE_HELLO_REPLY ^ 0x20): ethos->frametype = ETHOS_FRAME_TYPE_HELLO_REPLY; break;
                    case (ETHOS_FRAME_TYPE_HEART_BEAT ^ 0x20): ethos->frametype = ETHOS_FRAME_TYPE_HEART_BEAT; break;
                    case (ETHOS_FRAME_TYPE_UPLOAD_PROG ^ 0x20): ethos->frametype = ETHOS_FRAME_TYPE_UPLOAD_PROG; break;
                }
                ethos->state = IN_FRAME;
                break;
        }
...
int ethos_send_data(aboot_t *aboot, const uint8_t *data, size_t size) { return _send_frame(aboot, data, size, ETHOS_FRAME_TYPE_DATA); }

int ethos_send_text(aboot_t *aboot, const uint8_t *data, size_t size) { return _send_frame(aboot, data, size, ETHOS_FRAME_TYPE_TEXT); }
...
int ethos_init(aboot_t *aboot)
{
    ...
    if (aboot->stage != STAGE_INIT_DONE) return -1;
    ...
    if (aboot->stage != STAGE_HELLO_DONE) return -1;
...
static void smux_end_of_frame(aboot_t *smux)
{
    switch (smux->frametype)
    {
        case SMUX_FRAME_TYPE_STDIO:
            ...
            {
                if (smux->framesize == SMUX_PREAMBLE_SIZE && !strncmp((const char *)smux->rxbuf, SMUX_PREAMBLE_UABT, SMUX_PREAMBLE_SIZE))
                {
        ...
        case SMUX_FRAME_TYPE_HELLO_REPLY:
            ...
            break;
        case SMUX_FRAME_TYPE_ABOOT_CMD: aboot_transport_rx_cmd(smux, smux->rxbuf, smux->framesize); break;
        case SMUX_FRAME_TYPE_HEART_BEAT: break;
        case SMUX_FRAME_TYPE_ABOOT_DATA:
            // if(smux->data_cb) {
    ...
    for (i = 0; i < len; i++)
    {
        ...
        switch (smux->state)
        {
            ...
            case IN_ESCAPE:
                switch (c)
                {
                    case (SMUX_FRAME_DELIMITER ^ 0x20): smux_handle_char(smux, SMUX_FRAME_DELIMITER); break;
                    case (SMUX_ESC_CHAR ^ 0x20): smux_handle_char(smux, SMUX_ESC_CHAR); break;
                    case (SMUX_FRAME_TYPE_STDIO ^ 0x20): smux->frametype = SMUX_FRAME_TYPE_STDIO; break;
                    case (SMUX_FRAME_TYPE_HELLO ^ 0x20): smux->frametype = SMUX_FRAME_TYPE_HELLO; break;
                    case (SMUX_FRAME_TYPE_HELLO_REPLY ^ 0x20): smux->frametype = SMUX_FRAME_TYPE_HELLO_REPLY; break;
                    case (SMUX_FRAME_TYPE_ABOOT_CMD ^ 0x20): smux->frametype = SMUX_FRAME_TYPE_ABOOT_CMD; break;
                    case (SMUX_FRAME_TYPE_ABOOT_DATA ^ 0x20): smux->frametype = SMUX_FRAME_TYPE_ABOOT_DATA; break;
                    case (SMUX_FRAME_TYPE_HEART_BEAT ^ 0x20): smux->frametype = SMUX_FRAME_TYPE_HEART_BEAT; break;
                }
                smux->state = IN_FRAME;
                break;
        }
...
int smux_write_aboot_data(aboot_t *aboot, const uint8_t *data, size_t len) { return _send_frame(aboot, data, len, SMUX_FRAME_TYPE_ABOOT_DATA); }

int smux_write_aboot_cmd(aboot_t *aboot, const uint8_t *data, size_t len) { return _send_frame(aboot, data, len, SMUX_FRAME_TYPE_ABOOT_CMD); }
